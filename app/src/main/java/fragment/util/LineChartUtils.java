...
import fragment.MApplication;
import fragment.MyMarkerView;
...
public class LineChartUtils {
     ...
     * @return 初始化后的图表
     */
    public static LineChart initChart(Context context,LineChart chart, boolean isXEnbale, boolean isLeftYEnable, boolean isRightYEnable) {
        // 不显示表格颜色
        chart.setDrawGridBackground(false);
        chart.setDrawBorders(false);
        chart.setDragEnabled(true);//是否可以用手指移动图表
        chart.setScaleEnabled(true);//设置为false以禁止通过在其上双击缩放图表。是否可以缩放图表，当屏幕一屏显示不下，希望能通过缩放或滑动图表，仅设置这个和上面那个是不行的，还需要配合Matrix来实现
...
//        chart.getViewPortHandler().refresh(matrix,chart,false);
    ...
    public static void setLeftYAxisBasic(LineChart chart, boolean isEnable) {
        ...
        chart.getAxisLeft().setDrawGridLines(false);//是否画网格线
        chart.getAxisLeft().setGridColor(Color.GRAY);
        chart.getAxisLeft().setTextSize(8);//设置轴标签字体大小
        chart.getAxisLeft().setTextColor(Color.parseColor("#999999"));//设置轴标签字体的颜色
...
//        chart.moveViewToX(0f);//切换时间时用来将折线图回归到0点
    ...
    public static void notifyDataSetChanged(LineChart chart, List<Entry> values,
                                            List<String> datelist,int color1) {
...
//        chart.invalidate();
        setChartData(chart, values,LineDataSet.Mode.CUBIC_BEZIER,color1);
    }
    ...
    public static void setChartData(LineChart chart, List<Entry> values, LineDataSet.Mode mode,
        ...
        if (chart.getData() != null && chart.getData().getDataSetCount() > 0) {
            ...
            if (Utils.getSDKInt() >= 18) {
                ...
                Drawable drawable = ContextCompat.getDrawable(MApplication.context, color1);
